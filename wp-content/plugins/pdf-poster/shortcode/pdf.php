...
<div class="pdfp_main_wrapper" data-option='<?php echo wp_json_encode($options) ?>' style="width:100%; overflow:hidden;">
	...
	<?php
		if($default_browser === '1' && getBrowser() === 'Edge'){ ?>
			<iframe class="pdfp_frame" id="fra<?php echo $id; ?>" onload="disable<?php echo $id; ?>();" onMyLoad="disable<?php echo $id; ?>();" width="<?php echo $width['width'].$width['unit']; ?>" height="<?php echo $height['height'].$height['unit'];?>" src="<?php  echo $file_url;?>">
		</iframe>
		<?php }	else { ?>
			<iframe class="pdfp_frame" id="fra<?php echo $id; ?>" onload="disable<?php echo $id; ?>();" onMyLoad="disable<?php echo $id; ?>();" width="<?php echo $width['width'].$width['unit']; ?>" height="<?php echo $height['height'].$height['unit'];?>" src="<?php  echo $final_url;?>">
		</iframe>
		<?php } ?>
	</div>
</div>
